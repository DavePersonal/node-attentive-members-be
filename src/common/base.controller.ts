...
export class BaseController<T> implements IController <T> {
    ...
    async findAll(@QueryFilter() filter: IQueryFilter, @QueryPage() page: number, @QuerySize() size: number): Promise<PaginatedResult<T>> {
        return await this.baseService.findAll(filter, page, size)
    }
